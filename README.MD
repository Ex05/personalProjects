...
The Herder project aims to create a feature rich blazing fast media organization tool with multiple front end implementations, including a native C console application with true color support, a operating system independent Java GUI application and a HTML-5 based Web-App.

The project original started as a personal study exercise of the usage of modern C, advanced usage of preprocessor macros, low level memory management, multi-threading and processing, scalable I/O event notification in a GNU/Linux environment, secure network communication via 'TLS'. With the side goal of trying out the viability different git branching strategies.
<br/>
...
* Multithreaded handling of client connections using reusable threads through a custom thread pool implementation which utilises POSIXs parallel execution and communication model.
...
The NewsletterCreator project was an entry for a freelance job competition. Where the goal was to create a suite of software applications to move the creation and sending of advertisement emails In-house to reduce advertisement spending.
An additional factor was the ability to collect user data about whether emails were opened by recipients and which products led to click throughs to the web shop and later to sales.
...
And a GUI application to create new newsletters and manage previously send batches of emails.
...
The GUI-Application uses heavily customized javax.swing components to create its unique look and feel, but also incorporates modern javafx elements namely the javafx.scene.web.WebView to give a live preview inside the application of the final newsletter during the creation process.
...
* An easy to use framework to load and use animated bitmap based sprites through the defenition of animation presets.
</br>
...
* Support to manually or automatically created mipmaps for textures at the time of loading.
* Texture wrapping for objects with texture 
* Multiple language support for rendered text.
...
DeepMiner was created with the help of resources like the OpenGL SuperBible, the OpenGL Programming Guide, Oskar Veerhoek's LWJGL video series, and many others.

Utilising only OpenGL 1.1 functionality made accessible to the Java runtime by the Lightweight Java Game Library (LWJGL). DeepMiner makes all its rendering calls in OpenGLs immediate mode and has to resend all its rendering information every frame for every displayed object.
...
The game's collision detection code is based on zetcodes 2012 article on "Collision detection in 2D Java games.".
...
The functionality to change the games default textures or run modified versions of the game, which benefit from a larger heap size is also provided through the launcher.
